...
export class ShiprocketController {
  ...
  async createOrder(req: Request, res: Response) {
    try {
      ...
      const requiredFields = [
        'order_id', 'order_date', 'payment_method', 'order_items', 'sub_total',
        'billing_customer_name', 'billing_address',
        'billing_state', 'billing_country', 'billing_phone', 'billing_pincode',
      ...
      if (payload.shipping_is_billing === undefined || payload.shipping_is_billing === null) {
        missingFields.push('shipping_is_billing');
      }
      
      if (missingFields.length > 0) {
        console.error('[ShiprocketController] Missing required fields:', missingFields);
  ...
  async trackByAWB(req: Request, res: Response) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to track by AWB';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async trackByOrderId(req: Request, res: Response) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to track by order ID';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async getPickupLocations(req: Request, res: Response) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to fetch pickup locations';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async addPickupLocation(req: Request, res: Response) {
    try {
      console.log('[ShiprocketController] addPickupLocation', { pickup_location: req.body?.pickup_location });
      ...
      const message = error instanceof Error ? error.message : 'Failed to add pickup location';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async createVendorPickupLocation(req: Request, res: Response) {
    try {
      console.log('[ShiprocketController] createVendorPickupLocation', { vendor: req.body?.name || req.body?._id });
      const result = await this.shiprocketService.createVendorPickupLocation(req.body);
      res.json(result);
    } catch (error) {
      console.error('[ShiprocketController] createVendorPickupLocation error:', error);
      const message = error instanceof Error ? error.message : 'Failed to create vendor pickup location';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async updateVendorPickupLocation(req: Request, res: Response) {
    try {
      ...
      const result = await this.shiprocketService.updateVendorPickupLocation(vendorData, oldLocationName);
      res.json(result);
    } catch (error) {
      console.error('[ShiprocketController] updateVendorPickupLocation error:', error);
      const message = error instanceof Error ? error.message : 'Failed to update vendor pickup location';
      res.status(400).json({ success: false, error: message });
    }
  }
  ...
  async calculateRates(req: Request, res: Response) {
    try {
      ...
      const result = await this.shiprocketService.calculateShippingRates(rateRequestData);
      res.json({ success: true, data: result });
    } catch (error) {
  ...
  async trackOrderAdvanced(req: Request, res: Response) {
    try {
      ...
      if (type === 'awb') {
        ...
        trackingResponse = await this.shiprocketService.trackOrderById(parseInt(id));
      } else {
      ...
      const trackingActivities = trackingResponse.tracking_data?.shipment_track_activities || [];

      if (!shipmentTrack) {
        return res.status(404).json({ success: false, error: 'No tracking data found' });
      }

      const formattedTracking = {
        awb_code: shipmentTrack.awb_code,
        courier_name: (shipmentTrack as any).courier_name || `Courier ${shipmentTrack.courier_company_id}`,
      ...
      if (sendEmail) {
        console.log('[ShiprocketController] Email notification would be sent here');
      }
  ...
  async applyShippingRate(req: Request, res: Response) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to apply shipping rate';
      res.status(500).json({ success: false, error: message });
    }
  }
  ...
  async managePickupLocations(req: Request, res: Response) {
    try {
      ...
      console.error('[ShiprocketController] managePickupLocations error:', error);
      const message = error instanceof Error ? error.message : 'Failed to manage pickup locations';
      res.status(500).json({ success: false, error: message });
    }
  }
}
